...
public class BookingServiceImpl implements BookingService {
    ...
    public BookingServiceImpl(BookingRepository bookingRepository, JwtTokenProvider jwtTokenProvider, CarMapper carMapper, CarRepository carRepository, UserRepository userRepository, CustomerRepository customerRepository) {
    ...
    public MyRentalResponseDTO listBookings(HttpServletRequest request) {
        ...
        if (listBooking.isEmpty()) {
            ...
            for (Booking booking : listBooking) {
                ...
                carBookingBaseInfoDTO.setDeposit(booking.getCar().getDeposit());
    ...
    /**
     * Metho to get booking detail by id
     *
     * @param bookingId
     * @return
     */
     ...
     */
    @Override
    public FinishBookingDTO rentalCar(BookingInformationDTO bookingInformationDTO, HttpServletRequest request) {
       ...
       Customer customer = customerRepository.findById(customerId).orElse(null);
        ...
        if(bookingInformationDTO.getPaymentMethod().equals("My wallet")) {
            ...
            LocalDateTime endDate = booking.getEndDateTime().atZone(ZoneId.systemDefault()).toLocalDateTime();
            long numberOfDays = ChronoUnit.DAYS.between(starDate, endDate);
            BigDecimal total = car.getBasePrice().multiply(BigDecimal.valueOf(numberOfDays)); // tinh tien tong dua tren ngay thue xe
            BigDecimal deposit = car.getDeposit().multiply(BigDecimal.valueOf(numberOfDays));// tinh tien dat coc
        ...
        String message = String.format(
                "You've successfully booked %s from %s to %s%n%nYour booking number is: %d%n%nOur operator will contact you with further guidance about pickup.",
                car.getName(),
                booking.getStartDateTime(),
                booking.getEndDateTime(),
                booking.getId()
        );
        return new FinishBookingDTO(message);
